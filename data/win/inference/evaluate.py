...
def visualize_train_sample(
    ...
    if os.path.exists(model_path):
        ...
        try:
            ...
            return image_u8, label_color, pred_color, np.hstack([gt_overlay, gt_overlay]), hist_img, "❌ 可视化加载模型时显存不足，请先关闭其他任务或切到 CPU。", None, None, None
...
def evaluate_n_train_samples(
    ...
    try:
        ...
        if device.type == "cuda":
            torch.cuda.empty_cache()
        return None, None, None, None, None, "❌ 批量评估加载模型时显存不足，请先降低显存占用后重试。", None, None, None
