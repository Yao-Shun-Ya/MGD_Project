...
def preprocess_for_model(input_image):
    ...
    if isinstance(input_image, str):
        ...
        try:
            exif = img.getexif()
            if exif:
                orientation = exif.get(0x0112, 1)  # 0x0112是EXIF方向标签
                if orientation == 2:
    ...
    if input_image.ndim == 2:
        ...
        # Gradio numpy 默认是 RGB，这里不能按 BGR 处理。
...
def professional_training(
    ...
    if torch.cuda.is_available():
        ...
        if float(mem_limit) < 0.99:
            ...
            log_output += f"✅ 设备就绪: {gpu_name} (显存限额: {mem_limit*100}%)\n"
        else:
            log_output += f"✅ 设备就绪: {gpu_name} (显存限额: 100%，未强制限额)\n"
    ...
    if resume:
        ...
        if os.path.exists(best_model_path):
            # 先加载到 CPU，避免在显存紧张时因 map_location=cuda 直接 OOM。
    ...
    try:
        ...
        if device.type == "cuda":
            torch.cuda.empty_cache()
        log_output += "❌ 模型装载到 GPU 时显存不足。请尝试：显存限额=1.0、训练 Tile 高度降到 192/128、关闭断点续训后重试。\n"
        yield log_output
        return
...
# ==========================================
# 2. 核心架构：全尺度 U-Net (对标赛题)
# ==========================================
def get_clinical_model(device):
    """加载 31M 参数规模的满血版 U-Net"""
    model = UNet(
        spatial_dims=2,
        in_channels=1,
        out_channels=1,
...
def professional_inference(
    ...
    if os.path.exists(model_path):
        ...
        try:
            ...
            if device.type == "cuda":
                torch.cuda.empty_cache()
            return input_image, None, None, {"错误": "推理加载模型时显存不足，请调小 Tile 高度后重试"}
    else:
...
def visualize_train_sample(tile_infer, tile_height, tile_overlap, seg_threshold, min_area, smooth_fusion, smooth_sigma, roi_only):
    ...
    if os.path.exists(model_path):
        ...
        try:
            ...
            return image_u8, label_color, pred_color, np.hstack([gt_overlay, gt_overlay]), hist_img, "❌ 可视化加载模型时显存不足，请先关闭其他任务或切到 CPU。"
        model.eval()
        with torch.no_grad():
...
def evaluate_n_train_samples(num_samples, tile_infer, tile_height, tile_overlap, seg_threshold, min_area, smooth_fusion, smooth_sigma, roi_only):
    ...
    try:
        ...
        if device.type == "cuda":
            torch.cuda.empty_cache()
        return None, None, None, None, None, "❌ 批量评估加载模型时显存不足，请先降低显存占用后重试。"
    model.eval()
...
def apply_named_preset(preset_name):
    ...
    # 🌟 修复点：synced 本身已经包含了 16 个值，直接展开即可
    return (
        *synced,
...
with gr.Blocks(title="“腺”而易见 - 满血专业版") as app:
    ...
    with gr.Tabs():
        ...
        # --- Tab 2: 自动化训练舱 (恢复你的所有拉杆) ---
        with gr.TabItem("⚙️ 模型训练与微调"):
            with gr.Row():
                with gr.Column(scale=1):
                    gr.Markdown("### 🛠️ 训练超参数")
                    train_mem = gr.Slider(0.1, 1.0, value=1.0, step=0.1, label="显存限额")
