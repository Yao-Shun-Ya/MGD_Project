...
def professional_training(
    ...
    if torch.backends.mps.is_available():
        ...
        yield from log(f"✅ 设备就绪: {gpu_name} (MPS 加速)\n")
    ...
    try:
        ...
        if device.type == "cuda":
            ...
            yield from log("❌ 模型装载到 Apple Silicon GPU 时内存不足。请尝试：训练 Tile 高度降到 192/128、关闭断点续训后重试。\n")
        else:
            yield from log("❌ 模型装载时内存不足。请尝试：训练 Tile 高度降到 192/128、关闭断点续训后重试。\n")
        return
    ...
    use_amp = precision.startswith("FP16") and (device.type == "cuda" or device.type == "mps")
        ...
        for batch_idx, batch_data in enumerate(train_loader, 1):
            try:
                ...
                if device.type == "cuda":
                    ...
                    torch.mps.empty_cache()
                    yield from log("❌ 训练过程中 Apple Silicon GPU 内存溢出(OOM)。建议：1) 训练 Tile 高度降到 192 或 128；2) Batch Size=1；3) 必要时先关闭断点续训。\n")
                else:
                    yield from log("❌ 训练过程中内存溢出(OOM)。建议：1) 训练 Tile 高度降到 192 或 128；2) Batch Size=1；3) 必要时先关闭断点续训。\n")
                return
...
def simple_train(max_epochs=20, save_interval=2):
    ...
    if torch.backends.mps.is_available():
        device = torch.device("mps")
        print(f"✅ 算力引擎就绪: Apple Silicon GPU (MPS 加速)")
    elif torch.cuda.is_available():
        device = torch.device("cuda")
