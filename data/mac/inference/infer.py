    ...
    if os.path.exists(model_path):
        ...
        try:
            ...
            if device.type == "cuda":
                ...
                error_msg = "推理加载模型时显存不足，请调小 Tile 高度后重试"
            elif device.type == "mps":
                torch.mps.empty_cache()
                error_msg = "推理加载模型时 Apple Silicon GPU 内存不足，请调小 Tile 高度后重试"
            else:
                error_msg = "推理加载模型时内存不足，请调小 Tile 高度后重试"
            return input_image, None, None, {"错误": error_msg}, None, None, None
    else:
